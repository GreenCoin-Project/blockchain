...
module.exports = {
		convert_to_int: function(amount,cb){
			var fixed = amount.toFixed(0).toString(); 
			return cb(parseInt(fixed));
		},
        ...
        module.exports.is_unique(arr_vout, vout[i].scriptPubKey.addresses[0], function(unique, index) {
          if (unique == true) {
            ...
            module.exports.convert_to_satoshi(parseFloat(vout[i].value), function(amount_sat){
													//NILAM Changes
              arr_vout.push({addresses: vout[i].scriptPubKey.addresses[0], scriptPubKey: vout[i].scriptPubKey.hex, n: parseInt(vout[i].n),amount: amount_sat});
              loop.next();
            });
          } else {
            // already exists
          ...
          module.exports.is_unique(arr_vin, addresses[0].hash, function(unique, index) {
            if (unique == true) {
              module.exports.convert_to_satoshi(parseFloat(addresses[0].amount), function(amount_sat){
															//NILAM changes
                arr_vin.push({addresses:addresses[0].hash, txid: tx.vin[i].txid, amount:amount_sat});
                loop.next();
              });
            } else {
              module.exports.convert_to_satoshi(parseFloat(addresses[0].amount), function(amount_sat){
